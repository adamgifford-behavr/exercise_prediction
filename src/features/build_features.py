...
from ..data.make_dataset import _read_json
...
def _generate_featurize_id(
    ...
    # for that
    metadata = train_val_files["train"]
...
def check_for_existing_id(
    ...
    """
    with _session_scope(engine) as session:
        featurize_ids = session.query(table.featurize_id).distinct().all()
...
def main(
    ...
    Args:
      ...
      overwrite (bool): Whether to overwrite an existing dataset based on matching
    ...
    if prev_feat_id_exists and overwrite:
        logger.info(
            "previous features dataset exists but overwrite=True. deleting existing dataset..."
        )
        delete_existing_dataset(featurize_id, table, engine)
        logger.info("dataset deleted")

    if (not prev_feat_id_exists) or overwrite:
        ...
        for dataset_group, data_files in train_val_files.items():
            ...
            for file in data_files:
                df_file = pd.read_parquet(file, engine="fastparquet")
        ...
        for file in test_files:
            df_file = pd.read_parquet(file, engine="fastparquet")
            features_df = calculate_windowed_feats(
                featurize_id, "test", df_file, n_fft, features, all_features_df.columns
            )
            all_features_df = pd.concat([all_features_df, features_df])
